...
impl OptVirtualize {
    ...
    fn optimize_setfield_gc(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        let early = ctx
            .with_ptr_info_mut(struct_ref, |info| {
                if !info.is_virtual() {
                    return None;
                }
                if offset != Some(0) {
        ...
        if !ctx.has_ptr_info_via_box(struct_ref) {
    ...
    fn optimize_getfield_gc(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        if let Some(info) = ctx.peek_ptr_info_via_box(struct_ref) {
    ...
    fn optimize_arraylen_gc(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        if let Some(PtrInfo::VirtualArray(vinfo)) = ctx.peek_ptr_info_via_box(array_ref) {
    ...
    fn optimize_raw_load(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        if let Some(offset) = ctx.get_constant_int(offset_ref) {
            ...
            let (parent, base_offset) = match Self::resolve_raw_slice(buf_ref, ctx) {
                Some((p, o)) => (p, o),
                None if matches!(
                    ctx.peek_ptr_info_via_box(buf_ref),
    ...
    fn optimize_virtual_ref_finish(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        let did_forced_write = ctx
            .with_ptr_info_mut(vref_ref, |info| {
                if !info.is_virtual() {
                    return false;
                }
    ...
    fn optimize_jit_force_virtual(&mut self, op: &Op, ctx: &mut OptContext) -> bool {
        ...
        let forced_ok = match ctx.peek_ptr_info_via_box(forced_resolved) {
...
mod tests {
    ...
    fn test_standard_virtualizable_raw_first_read_is_not_cached() {
        ...
        let get_item = Op::with_descr(
            OpCode::GetarrayitemRawI,
            &[OpRef::input_arg_ref(0), OpRef::from_raw(50)],
            arr_descr.clone(),
        );
    ...
    fn test_setfield_getfield_on_virtual() {
        ...
        let mut ops = vec![
            ...
            Op::with_descr(
                OpCode::SetfieldGc,
                &[OpRef::input_arg_ref(0), OpRef::from_raw(100)],
                fd.clone(),
            ),
    ...
    fn test_new_array_virtual() {
        ...
        let mut ops = vec![
            ...
            Op::with_descr(
                OpCode::GetarrayitemGcI,
                &[OpRef::input_arg_ref(0), OpRef::from_raw(51)],
                ad.clone(),
            ), // pos=2
        ];
        assign_positions(&mut ops);
    ...
    fn test_guard_class_on_virtual() {
        ...
        let mut ops = vec![
            ...
            Op::new(
                OpCode::GuardClass,
                &[OpRef::input_arg_ref(0), OpRef::from_raw(200)],
            ),
        ];
        assign_positions(&mut ops);
    ...
    fn test_default_pipeline_forced_virtual_keeps_field_store_before_call() {
        ...
        let mut ops = vec![
            ...
            Op::new(
                OpCode::CallR,
                &[OpRef::from_raw(200), OpRef::input_arg_ref(0)],
            ),
    ...
    fn test_virtual_ref_non_escaping() {
        ...
        let mut ops = vec![
            ...
            Op::new(
                ...
                &[OpRef::input_arg_ref(0), OpRef::from_raw(102)],
            ), // pos=1
        ];
        assign_positions(&mut ops);
    ...
    fn test_virtual_ref_does_not_force_underlying_obj() {
        ...
        let mut ops = vec![
            ...
            Op::new(
                OpCode::VirtualRefR,
                &[OpRef::input_arg_ref(0), OpRef::from_raw(101)],
            ), // pos=1
    ...
    fn test_finish_forces_virtual_refs_to_emitted_allocations() {
        ...
        let mut ops = vec![
            ...
            Op::with_descr(
                OpCode::SetfieldGc,
                &[OpRef::from_raw(2), OpRef::input_arg_ref(0)],
                next_fd.clone(),
            ),
            Op::with_descr(OpCode::New, &[], node_sd.clone()),
            Op::with_descr(
    ...
    fn test_guard_fail_args_virtual_not_forced() {
        ...
        let mut ops = vec![
            ...
            Op::with_descr(
                OpCode::SetfieldGc,
                &[OpRef::input_arg_ref(0), OpRef::from_raw(10)],
                fd.clone(),
            ), // pos=1
            guard,                                                  // pos=2
        ];
    ...
    fn test_guard_fail_args_virtual_with_multiple_fields() {
        ...
        let mut ops = vec![
            ...
            Op::with_descr(
                OpCode::SetfieldGc,
                &[OpRef::input_arg_ref(0), OpRef::from_raw(20)],
                fd_b.clone(),
            ),
            guard,
        ];
